...
def test_ThingMessageViewSet__user_have_messages(db, thing_message_factory, api_client_with_credentials, username, password):
...
@pytest.mark.parametrize("username, password", [("user1", "password1"), ("user2", "password2")])
def test_ThingMessageViewSet__user_dont_have_messages(db, thing_message_factory, api_client_with_credentials, username, password):
    user = UserFactory(username=username, password=password)
    api_client_with_credentials.force_authenticate(user = user)
...
def test_post_ThingMessageViewSet__error(db, thing_message_factory, api_client_with_credentials):
...
def test_delete_ThingMessageViewSet__error(db, thing_message_factory, api_client_with_credentials):
    thing_message = thing_message_factory()
...
def test_update_ThingMessageViewSet__error(db, thing_message_factory, api_client_with_credentials):
    thing_message = thing_message_factory()
    url = reverse('thingmessage-detail', kwargs={'pk': thing_message.id})
...
def test_detail_ThingMessageViewSet__success(db, thing_message_factory, api_client_with_credentials, create_user):
    user = create_user
    api_client_with_credentials.force_authenticate(user = user)
