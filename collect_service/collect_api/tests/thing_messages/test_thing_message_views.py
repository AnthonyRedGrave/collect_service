...
from collect_api.tests.factories import ThingMessageFactory
...
def test_ThingMessageViewSet__user_have_messages(db, thing_message_factory, api_client, create_user):
    user = create_user
...
def test_ThingMessageViewSet__user_dont_have_messages(db, thing_message_factory, api_client, create_user):
    user = create_user
    api_client.force_authenticate(user = user)
    thing_message_factory.create_batch(20, user=user)
    url = reverse('thingmessage-list')
